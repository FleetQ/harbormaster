...
| `orchestrator`     | `str` | `"auto"` | v27.0.0. Which calling MCP client (orchestrator) instruction-mode packets are rendered for, selecting the packet adapter. `"auto"` (default) detects the caller from the MCP `clientInfo` handshake, falling back to `"claude"` when undetected (preserves the v26 packet byte-for-byte). `"claude"` / `"codex"` / `"gemini"` / `"neutral"` pin the adapter so non-Claude callers (Codex CLI, Gemini CLI) get a packet in their own sub-agent idiom. Callers can override per-call via the tools' `orchestrator` parameter. An unknown value falls back to subprocess execution. SSH targets always force `subprocess` regardless. |
| `subprocess_timeout` | `int` | `null` | v27.1.3. > 0. **Authoritative** wall-clock (seconds) for **local** async subprocess-mode delegate jobs run by the JobWorker (`run_backend` → `ask_local` → `subprocess.run(timeout=…)`). When set, it OVERRIDES `[backends.<name>].timeout_local` for those jobs only — the interactive sync `delegate_task` / `ask_project` / `fan_out_ask` paths still use `timeout_local`. `null` (default) preserves v22-v27 behaviour (async local jobs inherit `timeout_local`, default 300 s). Remote (SSH) async jobs are unaffected — they are governed by `[hosts.<label>].total_timeout` because `ask_remote` ignores backend timeouts. Read once at process boot: an MCP server restart is required to pick up a change. Set e.g. `subprocess_timeout = 5400` to allow 90-minute local delegate jobs. |
